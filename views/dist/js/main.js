...
var resizePizzas = function(size) { 
 window.performance.mark("mark_start_resize");   // User Timing API function
  ...
  // Returns the new relative width for pizzas Called by changePizzaSizes(size).
...
function sizeSwitcher (size) {
 ...
 // Avoid colsunting the dom creating a dom array
 // Calculate witdth outside the loop
 // Use relative widths
...
/**   I've created a vriable scroll_ in order to calculate the scrollTop distance outside the loop*/
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    var x_translate = 90 * phase;
    items[i].style.transform = "translateX("+x_translate+"px)";
  }
...
window.addEventListener('scroll',updatePizzas);

// wrap updatePositions to make a requestAnimationFrame
function updatePizzas(){window.requestAnimationFrame(updatePositions);
};
